...
type OrderData = {
  ...
  orderDate: string
  stones: { stoneType: string; quantityWeight: string; unitPrice: number | null; girdleCode: string | null; stoneNote: string | null }[]
  materials: { category: string; quantityWeight: string; gemSize: string | null; girdleCode: string | null; materialNote: string | null }[]
}
...
export function EditOrderForm({
  ...
  const [storeId, setStoreId] = useState(String(order.storeId))
  const [factoryId, setFactoryId] = useState(String(order.factoryId))
  const [customerName, setCustomerName] = useState(order.customerName)
  const [customerPhone, setCustomerPhone] = useState(order.customerPhone)
  const [category, setCategory] = useState(order.category)
  const [size, setSize] = useState(order.size ?? '')
  const [laborFee, setLaborFee] = useState(order.laborFee != null ? String(order.laborFee) : '')
  const [goldPrice, setGoldPrice] = useState(order.goldPrice != null ? String(order.goldPrice) : '')
  const [materialDesc, setMaterialDesc] = useState(order.materialDesc ?? '')
  const [orderDate, setOrderDate] = useState(order.orderDate)

  // Merge all old notes into one remarks field
  const existingRemarks = [order.customNotes, order.styleNotes, order.remarks].filter(Boolean).join('\n')
  ...
  const [stones, setStones] = useState<Stone[]>(
    order.stones.length > 0
      ? order.stones.map((s) => ({
          ...
          girdleCode: s.girdleCode ?? '',
          stoneNote: s.stoneNote ?? '',
        }))
      : [{ stoneType: '', quantityWeight: '', unitPrice: '', girdleCode: '', stoneNote: '' }]
  )
  const [materials, setMaterials] = useState<Material[]>(
    order.materials.map((m) => ({
      ...
      gemSize: m.gemSize ?? '',
      girdleCode: m.girdleCode ?? '',
      materialNote: m.materialNote ?? '',
    }))
  )
